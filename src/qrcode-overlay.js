...
function create_canvas_element(url) {
    var c = QR.canvas = document.createElement('canvas');
    c.width = c.height = QR.size = 300;
    QR.draw(url);

    c.classList.add('qr-canvas');
...
function init_QR() {
    ...
    fetch('/server_info')
        .then(r => r.json())
        .then(d => {
            const url = d.origin + location.pathname;
            create_canvas_element(url);
        })
        .catch(err => {
